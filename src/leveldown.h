...
static inline leveldb::Slice StringOrBufferToSlice(v8::Local<v8::Value> from) {
  size_t sz_;
  char* ch_;
  ...
  if (from->IsNull() || from->IsUndefined()) {
    ...
    sz_ = node::Buffer::Length(from->ToObject());
    ch_ = node::Buffer::Data(from->ToObject());
  } else {
...
#define LD_METHOD_SETUP_COMMON_CBNULL(name, optionPos, callbackPos)            \
  bool hasCallback = info[callbackPos]->IsFunction();                          \
  leveldown::Database* database =                                              \
    Nan::ObjectWrap::Unwrap<leveldown::Database>(info.This());                \
  v8::Local<v8::Object> optionsObj;                                            \
  v8::Local<v8::Function> callback;                                            \
...
#define LD_METHOD_CHECK_DB_ERROR(name)                    \
  if (!status.ok()) {                                \
    Status* st = reinterpret_cast<Status*>(&status); \
    return Nan::ThrowError(                          \
      Nan::ErrnoException(st->code(), #name, status.ToString().c_str()) \
    ); \
  }

#define LD_METHOD_SETUP_SIMPLE(name, minPos, optionPos)   \
  if (info.Length() <= minPos)                                                      \
    return Nan::ThrowError(Nan::ErrnoException(kInvalidArgument, #name, #name "() miss arguments"));           \
  leveldown::Database* database =                                              \
    Nan::ObjectWrap::Unwrap<leveldown::Database>(info.This());                 \
  v8::Local<v8::Object> optionsObj;                                            \
  if (optionPos != -1                                                   \
