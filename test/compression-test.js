...
var async      = require('async')
  ...
  , common     = require('abstract-nosql/testCommon')
  , leveldown  = require('../')
...
test('set up', common.setUp)

test('test data is compressed by default (db.put())', function (t) {
...
test('test data is not compressed with compression=false on open() (db.put())', function (t) {
  var db = leveldown(common.location())
...
test('test data is compressed by default (db.batch())', function (t) {
  var db = leveldown(common.location())
  db.open(function (err) {
    t.error(err)
    db.batch(
